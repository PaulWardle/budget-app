...
- Transactions whose description ends "(expected)" are presumed bill postings: the app charges each bill against the balance on its due date, then replaces the estimate with the real payment (or removes it) when the next statement is uploaded — like an estimated meter reading. Account balances already include them; treat them as bills paid, but flag them as expected rather than confirmed if the user asks about one specifically.
